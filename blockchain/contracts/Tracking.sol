...
struct Shipment {
    ...
    event ShipmentCreated( address indexed sender, address indexed reciever, uint256 pickupTime,
     ...
     function CreateShipment( address _reciever, uint256 _pickupTime, uint256 _distance,
      uint256 _price ) public  payable{
         ...
         Shipment memory shipment = Shipment (msg.sender, _reciever, _pickupTime ,  0 , 
     ...
     function CompleteShipment(address _sender, address _reciever, uint256 _index) public {
        ...
        require ( shipment.reciever == _reciever, "Invalid reciever.");
        require (shipment.status == ShipmentStatus.IN_TRANSIT, "Shipment ot in transit." );
